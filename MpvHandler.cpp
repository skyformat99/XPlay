//声明：此文件中的所有代码均摘录自“https://github.com/u8sand/Baka-MPlayer/blob/master/src/mpvhandler.cpp”，并做了部分改动
...
static void wakeup(void *ctx)
{
    ...
    emit mpvhandler->mpv_events();
}

MpvHandler::MpvHandler(int64_t wid, QObject *parent) : QObject(parent)
{
    // create mpv
    mpv = mpv_create();
    if(!mpv)
    {
        throw std::runtime_error("Could not create mpv object");
    }
...
void MpvHandler::Volume(int level, bool osd)
{
    ...
    if(playState > 0)
    {
        ...
        if(osd)
        {
            ShowText(tr("Volume") + QString(" ") + QString::number(level) + QString("%"));
        }
    }
    else
    {
...
void MpvHandler::ShowText(QString text)
{
    const char *args[] = {"show_text", text.toUtf8().constData(), NULL};
    AsyncCommand(args);
}
